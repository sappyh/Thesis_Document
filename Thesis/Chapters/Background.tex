...
The FX3 buffers are controlled by the Cypress FX3 USB Controller using  \ac{GPIF} II.
...
LimeSDR-USB uses a Cypress EZ-FX3 as USB 3.0 peripheral controller.
The Cypress EZ-FX3 has a fully configurable, general programmable interface called the \ac{GPIF} II .
It allows the EZ-FX3 to integrate with any processor like \ac{ASIC}, \ac{FPGA}, Image Sensors etc.
It also provides low speed interfaces like \ac{I2C}, \ac{SPI} for the low-speed IO operations.\\
...
\label{FX3_arch}
\end{figure}
The architecture of EZ-FX3 is shown in Figure \ref{FX3_arch}.
The "ARM926EJ" is a 32-bit processor operating at 200 MHz, it is responssible for the configuring and controlling all the data tranfers through the Dstributed DMA Controller.
The "USB" is the \ac{USB} 3.0 peripheral controller, it includes the USB 3.0 Physical Layer.
It is responsible for handling the communication with the Host Computer.
The \ac{GPIF} controller handles the data streams to and from the \ac{FPGA} of the LimeSDR-USB. 
The controllers the Synchronous Slave Interface implemented on the \ac{FPGA}.
...
The control endpoints are used for configuring and retrieving data from the LMS7002M, where are data endpoints are used for streaming data to the LMS7002M data converters through the \ac{FPGA}.
...
They don't have any guarantees on the bandwidth and are transmitted when they is bandwidth available after other types of transfers.
They offer guaranteed delivery of data.\\

The LimeSDR-USB uses Bulk Transfers for the control transfers.
It sends bursts of data which are moved in the endpoint buffer of the Host Computer USB Controller which transfers it to the endpoint point buffer of "USB" block shown in Figure \ref{FX3_arch}.\\

For the data transfers, the LimeSDR-USB uses bulk streams.
USB 3.0 protocol introduced stream pipes where a single endpoint can be allocated to multiple data streams.
Each stream endpoint has their associated Stream ID which is used for tagging the data in the USB endpoint packet buffer.
Each of the data packets the endpoint buffer are transmitted as a regular bulk transfer and the USB Controller on the other end decodes the Stream ID and send it to the appropriate Stream endpoint. \\

USB endpoints specifies the size of the data packet.
...
The GPIF II segments the functionality into control and data part.
For the data part, the LimeSDR-USB uses a 32 bit interface running at 100MHz.
...
The Slave FIFO interface allows the \ac{FPGA} to perform read/ write operations directly to the EZ-FX3 internal FIFO buffers.
The Control signals are used for addressing the internal FIFO buffers and signaling read and write operations.
In addition, the control signals has flag signals to indicate events to the FIFO Slave Interface.
The 32-bit data interface transfers the data according to the signaling and addressing used by the control signals.
